...
(function($) {
    ...
    $.extend(Task.prototype, {
        ...
        login: function() {
            ...
            form.find("#login-btn").click(function(e) {
                ...
                if (form.find("#inputPassword").length) { // for compatibility with previous versions
                ...
                if (ready) {
                    self.send_request('login', [form_data], function(success) {
                        if (success) {
                            ...
                            form.find("#login-error-msg").removeClass("hidden");
                        }
                    });
                }
            });
            ...
            form.find("input").focus(function() {
                ...
                form.find("#login-error-msg").addClass("hidden");
            });
    ...
    // Item interface methods
        ...
        calc_summary: function(detail, fields, callback, summary_fields) {
            ...
            if (this.on_detail_changed) {
                console.log("on_detail_changed1")
                if (fields instanceof Array && fields.length) {
                    console.log("on_detail_changed2");
                    for (i = 0; i < fields.length; i++) {
    ...
    DBTable.prototype = {
        ...
        create_table: function() {
            const table_name = this.item.item_name;
            var self = this,
                $doc = $(document),
                $selection,
                $th,
            ...
            this.$element.find('.table-container').append($(
                '<table id=outer-' + table_name + ' class="outer-table table table-condensed table-bordered" style="width: 100%;">' +
                '   <thead>' +
                '       <tr id="header-row"><th>&nbsp</th></tr>' +
                '   </thead>' +
                '   <tr>' +
                ...
                '       <tr id="footer-row"><th>&nbsp</th></tr>' +
                '   </tfoot>' +
                '</table>'));
        ...
        fill_title: function($element) {
            ...
            // heading = $element.find("thead tr:first");
            // heading = document.getElementById("header-row");
            $heading = $element.find("#header-row");
            $heading.empty();
            if (this.options.multiselect) {
                if (this.item.master || !this.item._paginate) {
                    if (this.selections_get_all_selected()) {
                    ...
                    // div = $('<div class="text-center multi-select" style="overflow: hidden"></div>');
                    // sel_count = $(
                    //   '<p class="sel-count text-center">' +
                    //     this.item.selections.length +
                    //     "</p>")
                    // div.append(sel_count);
                    // input = $('<input class="multi-select-header" type="checkbox" ' + checked + ' tabindex="-1">');
                    // div.append(input);
                    // cell = $('<th class="multi-select-header"></th>').append(div);
                    $cell = `<th class="multi-select-header"> \
                                <div class="text-center multi-select" style="overflow: hidden"> \
                                    <p class="sel-count text-center">${this.item.selections.length}</p> \
                                    <input class="multi-select-header" type="checkbox" ${checked} tabindex="-1"> \
                                </div> \
                           </th>`;
                    $heading.append($cell);
                }
                    ...
                    $input = $bl.find("#mselect-all");
                    $bl.click(function(e) {
                        const target = e.target;
                        switch (target.innerHTML) {
                          case "Select all":
                            e.preventDefault();
                            self.selections_set_all_selected_ex(
                              true
                            );
                            self.$table.focus();
                            break;
                          ...
                          case "Show selected":
                            e.preventDefault();
                            self.item._show_selected = !self
                              .item._show_selected;
                            self.item._update_page(
                              {
                                __show_selected_changed: true
                              },
                              function() {
                                self.selections_update_selected();
                                self.$table.focus();
                    ...
                    $input.css(
                        {
                            'top': $sel_count.outerHeight() + $sel_count.position().top + 4,
                            'left': ($cell.outerWidth() - $input.width()) / 2 + 1
                        }
                    );
                    // $cell[0].style.paddingTop = 0;
                    // $input[0].style.top = $sel_count[0].offsetHeight + $sel_count[0].offsetTop + 4;
                    // $input[0].style.left = ($cell[0].offsetWidth - $input[0].style.width) / 2 + 1
                    $heading.append($cell);
                    $cell.append($bl);
                }
            }
            ...
            for (i = 0; i < this.fields.length; i++) {
                ...
                const title_height = '' ? this.options.title_word_wrap : `height: ${this.text_height}px`
                const th = `<th class="${field.field_name}" data-field_name="${field.field_name}"> \
                                <div class="text-center ${field.field_name}" style="overflow: hidden;${title_height}"> \
                                    <p style="margin: 0">${caption}</p> \
                                </div> \
                            </th>`;
                ths.push(th)
            }
            ths.push('<th class="fake-column" style="display: None;></th>')
            $heading.append(ths.join(""));
            if (this.options.title_callback) {
                this.options.title_callback($heading, this.item)
            }
            this.selections_update_selected();
            console.timeEnd('fill_title');
        },
        ...
        fill_footer: function($element) {
            ...
            // footer = $element.find("tfoot tr:first");
            footer = $element.find("#footer-row");
            old_footer = footer.clone();
            footer.empty();
            let th = ''
            let th_div_html = " ";
            const ths = []
            if (this.options.multiselect) {
                th = `<th class="multi-select"> \
                          <div class="text-center multi-select" style="overflow: hidden; width: 100%"> \
                          </div> \
                      </th>`;
                ths.push(th)
                // footer.append(cell);
            ...
            for (i = 0; i < len; i++) {
                field = this.fields[i];
                old_div = old_footer.find('div.' + field.field_name)
                th_div_html = old_div.html() ? (old_div.length || old_div.html() !== '&nbsp') : '&nbsp';
                th = `<th class="${field.field_name}"> \
                          <div class="text-center ${field.field_name}" style="overflow: hidden; width: 100%">&nbsp</div> \
                      </th>`;
                ths.push(th)
            }
            footer.append(ths.join(''));
            if (!this.options.show_footer) {
                footer.hide();
            }
        },
        ...
        show_footer: function() {
            this.$element.find("#footer-row").show();
            // this.$element.find("table.outer-table tfoot tr:first").show();
        },

        hideFooter: function() {
            this.$element.find("footer-row").hide();
        ...
        sync_col_width: function(all_cols) {
            const stamp = Math.random()
              .toString(36)
              .substring(7);
            console.time("sync_col_width Long" + stamp);
            var $row,
                field,
                $th;
            if (this.item.record_count()) {
                ...
                if (this.fields.length && this.item.visible) {
        ...
        fill_datasource: function(start_rec) {
            const stamp = Math.random()
              .toString(36)
              .substring(7);
            console.time("fill_datasource" + stamp);
            var self = this,
                counter = 0,
                clone = this.item.clone(true);
            ...
            console.timeEnd("fill_datasource" + stamp);
        },

        fill_rows: function() {
            console.time('fill_rows')
            var i,
                len,
                row,
                rows,
        ...
        refresh: function() {
            ...
            if (this.auto_field_width && !this.field_width_updated) {
                ...
                tmpRow = '<tr>'
                if (this.options.multiselect) {
                    tmpRow += '<th class="multi-select">' +
                                '<div class="text-center multi-select" style="overflow: hidden"></div>' +
                              '</th>';
                }
                len = this.fields.length;
                for (i = 0; i < len; i++) {
                    tmpRow +=  '<th class="' + this.fields[i].field_name + '" ><div style="overflow: hidden">' +
                ...
                // tmpRow[0].parentNode.removeChild(tmpRow[0]);
                tmpRow.remove();
            ...
            if (!this.item.rec_count) {
                ...
                const outer_table_style = this.$outer_table.prop(
                  "style"
                );
                this.$head.find("th").css({
                  borderBottomWidth:
                    outer_table_style.borderTopWidth,
                  borderBottomStyle:
                    outer_table_style.borderTopStyle,
                  borderBottomColor:
                    outer_table_style.borderTopColor
                });
            }
            console.timeEnd("refresh" + stamp);
        },
